...
function createUser(kuzzle, name, profile, userInfo) {
  ...
  userInfo._id = name;
  userInfo.profileIds = kuzzle.config.security.restrictedProfileIds;
  if (profile) {
...
function registerStrategy(Strategy, name, context, verify, options = {}) {
  ...
  try {
    ...
    if (this.pluginsManager.registeredStrategies.indexOf(name) === -1) {
      this.pluginsManager.registeredStrategies.push(name);
    }
  }
  catch (e) {
